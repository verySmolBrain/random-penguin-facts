...
var penguin_app_entry = (() => {
  var _scriptDir = import.meta.url;
  
  return (
async function(penguin_app_entry = {})  {
...
var Module = typeof penguin_app_entry != "undefined" ? penguin_app_entry : {};
...
[ "__emscripten_thread_init", "__emscripten_thread_exit", "__emscripten_thread_crashed", "__emscripten_thread_mailbox_await", "__emscripten_tls_init", "_pthread_self", "checkMailbox", "establishStackSpace", "invokeEntryPoint", "PThread", "_fflush", "__embind_initialize_bindings", "__emscripten_check_mailbox", "onRuntimeInitialized" ].forEach(prop => {
...
if (Module["locateFile"]) {
 wasmBinaryFile = "penguin_app.wasm";
 if (!isDataURI(wasmBinaryFile)) {
  wasmBinaryFile = locateFile(wasmBinaryFile);
 }
} else {
 wasmBinaryFile = new URL("penguin_app.wasm", import.meta.url).href;
}
...
var PThread = {
 ...
 allocateUnusedWorker: function() {
  var worker;
  if (!Module["locateFile"]) {
   worker = new Worker(new URL("penguin_app.worker.cjs", import.meta.url));
  } else {
   var pthreadMainJs = locateFile("penguin_app.worker.cjs");
...
function ___emscripten_init_main_thread_js(tb) {
 __emscripten_thread_init(tb, !ENVIRONMENT_IS_WORKER, 1, !ENVIRONMENT_IS_WEB, 5242880);
...
function ___syscall_chmod(path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(4, 1, path, mode);
 try {
...
function ___syscall_ftruncate64(fd, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 1, fd, length);
 try {
...
function ___syscall_getcwd(buf, size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(10, 1, buf, size);
 try {
  if (size === 0) return -28;
  var cwd = FS.cwd();
...
function ___syscall_rmdir(path) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 1, path);
 try {
  path = SYSCALLS.getStr(path);
  FS.rmdir(path);
  return 0;
...
function ___syscall_symlink(target, linkpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 1, target, linkpath);
 try {
...
function ___syscall_truncate64(path, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 1, path, length);
 try {
  length = bigintToI53Checked(length);
  if (isNaN(length)) return -61;
  path = SYSCALLS.getStr(path);
...
function ___syscall_unlinkat(dirfd, path, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(22, 1, dirfd, path, flags);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  if (flags === 0) {
...
function ___syscall_utimensat(dirfd, path, times, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 1, dirfd, path, times, flags);
 try {
  path = SYSCALLS.getStr(path);
  assert(flags === 0);
...
function __mmap_js(len, prot, flags, fd, off, allocated, addr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 1, len, prot, flags, fd, off, allocated, addr);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(25, 1, addr, len, prot, flags, fd, offset);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (prot & 2) {
...
function _emscripten_get_element_css_size(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(26, 1, target, width, height);
...
function _environ_get(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 1, __environ, environ_buf);
 var bufSize = 0;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function _fd_fdstat_get(fd, pbuf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 1, fd, pbuf);
 try {
  var rightsBase = 0;
...
function _fd_seek(fd, offset, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 1, fd, offset, whence, newOffset);
 try {
...
function _fd_sync(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(36, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
var wasmImports = {
 ...
 "__asyncjs__qt_jspi_suspend_js": __asyncjs__qt_jspi_suspend_js,
 ...
 "init_jspi_support_js": init_jspi_support_js,
 "invoke_fi": invoke_fi,
 "invoke_i": invoke_i,
 "invoke_ii": invoke_ii,
 ...
 "jsHaveJspi": jsHaveJspi,
 "memory": wasmMemory || Module["wasmMemory"],
 "qt_asyncify_resume_js": qt_asyncify_resume_js,
 "qt_asyncify_suspend_js": qt_asyncify_suspend_js,
 "qt_jspi_can_resume_js": qt_jspi_can_resume_js,
 "qt_jspi_resume_js": qt_jspi_resume_js,
...
var ___start_em_js = Module["___start_em_js"] = 8408996;

var ___stop_em_js = Module["___stop_em_js"] = 8410086;
...
run();
  ...
  return penguin_app_entry.ready
}

);
})();
export default penguin_app_entry;
...
This works ok with Typescript Jest (Once you modify your node environment to use CommonJS) but not so well on the actual frontend. I've tried to update the .js files to .cjs files manually but I still get import errors from vite when it actually tries to dynamically load it. I'm not sure why this is the case since Vite should technically work fine with importing .cjs files but this might be a compatibility issue between generated code from Emscripten and the Vite bundler. There's a potential path to look into with using `ENVIRONMENT` flag when building for Emscripten which might solve it though so far everything is working for ES6 for me so I haven't looked too deeply into this. I've seen some articles about either using Vite transform or WebPack that might make it compatible but that's WIP.

### WIP Webpack

Note that from what I've read, it seems you can setup a proper configuration of multi-threaded Emscripten using web workers with a few workarounds in Webpack whereas with Vite, this isn't possible yet due to a bug involving web worker imports. This involves editing the configuration for worker imports and replacing them with something else. Testing this out is WIP.
...
- The `ENVIRONMENT=''` flag is nice to set for maximum portability. I made a mistake of accidentally setting `ENVIRONMENT='web,worker'` and then wondering why it didn't work in a node environment. Don't be like me.

- It's also nice to set `EXPORT_NAME` to something other than `Module` because I ran into a few issues with a polluting global namespace and it makes things a bit unclear. Change it to something like `createModule` for clarity if you're also using the `Modularize` flag. Note that if you do use `EXPORT_NAME` or change it to something other than `Module`, you'll have to modify the .html file Emscripten generates if you do generate the .html file.
...
Personally I use both ESM + TypeScript because this is an experimental project and trying to set it up to support this combination was a good learning experience. I've also noted that having TypeScript compile to ESM with module set makes it very easy to port things over to Vite which is the frontend tooling I'm using.

For testing, Jest does not work well with anything other than standard JavaScript with CommonJS requires imports. You'll need to install ts-jest or have a Babel setup for Typescript support. If you want to setup Typescript + ESM imports with ts-jest you should have a jest.config.js file that looks something like this:
...
Now with frontend I've just used a standard default Vite + React setup that's been working well for the most part. 

Note that I personally use Vite because of the ease of setup. It uses Rollup for production build, ESBuild for dev builds and ESModules with Typescript as an opinionated setup. Since most of my experience with frontend has been small mini-projects where configuration isn't very important, this is the frontend tooling that I'm used to working with as everything just works out of the box. For a proper development setup for a large-scale webapp, you'd probably want something more battletested and configurable like Webpack. Personally I haven't used Webpack outside of the default settings using CRA (create-react-app) and the configuration options and documentation for Webpack is a bit overwhelming. I haven't tried testing other bundlers but this might be something interesting to look into. To get CORS working on Vite simple append this:
...
Here's an explanation for the flags I use. I've added `--no-entry` so main isn't called automatically since I'm mainly playing around with libraries right now instead of generating an actual executable that needs to be run each time. This means main isn't called. I also use `-s EXPORT_ES6=1` which requires `-s MODULARIZE=1` because I mainly work with ES6. Explanation for `-s EXIT_RUNTIME=1` should be [here](#emcc--multithreaded--esm).
...
Note I've found a few paths that might be promising. First is completely changing the way the web worker is instantiated by manually editing the .js files it outputs. So far even doing this manually, I haven't been able to get this to work but I'm also not familiar with web worker instantiation. Another option is to try out different Emscripten flags for building. There's a lot of possible variations you can try such as `BUILD_AS_WORKER=1` that might seem promising. Personally I haven't been able to find a combination that works for every setup but this might be something to look into. An alternative is to just ditch the .js glue code completely and stick with the .wasm file that is output. This provides more control over what is happening but at the same time, it's also not easy to setup since you'll have to directly interact with the .wasm file and the .js glue code generated by Emscripten covers a lot of edge cases.

For Emscripten make sure you load the .js file it outputs with async because it needs time to instantiate the .wasm file. The way you do this depends on which flags you use for the project and where you're loading it eg. in a Node environment vs HTML environment vs React environment. An important distinction is between using or not using the `modularize=1` flag. Without the `modularize=1` flag, Emscripten generates the script in the global scope with default name `Module` unless you've set it to something else with `EXPORT_NAME`. You then simply call `.onRunTimeInitialised` on the global Module object (This is essentially a function from the Module wrapper class generated by Emscripten). If you use `modularize=1` then you can initialise multiple instances of the WASM object which means you should instead just use `await MyModuleName()` instead. Note that the default generated HTML file does not support this and you'll have to modify it. I've found that once you get a proper Frontend setup like React and have a generated workflow the HTML file becomes largely irrelevant. 

A common issue you'll find documented is CORS issue with SharedArrayBuffer if you decide to use pthreads and web workers (Personally I've struggled to get web workers running with QT + Emscripten and have it running on the frontend without something breaking). A solution if you want to play with standard HTML files is the `qtwasmserver.py` that you can download from Python pip. Emscripten also has a built in tool for this. I've talked about how to resolve this easily in Vite + React in [Frontend](#es6--typescript--frontend-reflection) without needing to setup certificates or download mkcert.
...
For abstract classes, you can use either `.extend()` or `.implement()` to create a JavaScript class that inherits from the C++ class. Either way works but they're suited for different things. You can read more about this at the [official documentation for Embind](https://emscripten.org/docs/porting/connecting_cpp_and_javascript/embind.html#classes). Note I ran into an issue early on where I used the wrong Embind macro as my class function wasn't a pure virtual and received this error:
...
- Installation instructions for other OS (Especially windows)
